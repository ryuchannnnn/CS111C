...
public class HomeworkM11Driver {
	...
	public static void splice(QueueInterface firstQueue, QueueInterface secondQueue) {
		// check if secondQueue is not empty, if empty do nothing and return
		if(!secondQueue.isEmpty())
		{
			// if secondQueue is not empty
			// create a new temporary queue tempQueue
			QueueInterface tempQueue = new LinkedQueue();
			// loop that continues till secondQueue is empty
			while(!secondQueue.isEmpty())
			{
				firstQueue.enqueue(secondQueue.getFront()); // retrieve and insert the front element of secondQueue to firstQueue
				tempQueue.enqueue(secondQueue.dequeue()); // remove and insert the front element of secondQueue to firstQueue
			}

			// loop that continues till tempQueue is not empty
			// loop to rebuild the secondQueue so that it os back to its original state
			while(!tempQueue.isEmpty())
			{
				secondQueue.enqueue(tempQueue.dequeue()); // remove and insert the front element of tempQueue to secondQueue queue
			}
		}  
	}
