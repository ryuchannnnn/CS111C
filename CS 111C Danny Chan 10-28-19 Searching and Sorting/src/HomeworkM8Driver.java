...
public class HomeworkM8Driver {
	...
	{
		if(array.length==1 || array.length==0)   return 1;

		float sortedCount = 0;
		float unSortedCount = 0;
		float equalCount = 0;
		...
		return sortedCount/(sortedCount+unSortedCount);
	}

	public static double sortedness(Node<Comparable> node) {
		// NOTE: If you want to use a local NodeT variable,
		// declare it as type NodeT<Comparable>
		...
		Node<Comparable> n = node;
		Node<Comparable> n2 = null;
		while (n != null)
		{
			Comparable data1 = n.data;
			n = n.next;

			n2 = n;
			if(n2 != null)
			{
				Comparable data2 = n2.data;
				n2 = n2.next;
