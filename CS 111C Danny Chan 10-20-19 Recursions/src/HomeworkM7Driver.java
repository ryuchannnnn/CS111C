...
public class HomeworkM7Driver {
	...
	else if (word.substring(0,1).matches("q")&&word.length()==1) 
	{
		return true;
	}
	return qNotFollowedByUHelper(word, 0);
	}
	public static boolean qNotFollowedByUHelper(String word, int counter) {
		if (!word.isEmpty() && counter+1 < word.length() && word.contains("q")) {
			if (word.substring(counter,counter+1).equals("q")) 
			{
				if (!word.substring(counter+1,counter+2).matches("u")) 
				{
					return true;
				}
			} 
			else if (word.endsWith("q")) 
			{
				return true;
			} 
			else 
			{
				qNotFollowedByUHelper(word, counter + 1);
			}
			return false;
		}
		return true;
