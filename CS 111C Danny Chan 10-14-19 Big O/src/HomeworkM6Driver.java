...
public class HomeworkM6Driver {
	...
	public static List<Integer> findDuplicatesLinear(List<Integer> numbers) {
		//We are using Set to store first time number, next time on words if that number repeats we are storing it in duplicateList
		//To search for a number whether or not exist in the HashSet will take O(1) time, which means constant time.
		Set<Integer> initialValues = new HashSet<Integer>();
		List<Integer> duplicateList = new ArrayList<Integer>();

		//Looping through all the numbers once
		for (int i = 0; i < numbers.size(); i++) {
			//Checking number is exist in the intialValues or not. If exists we are storing it in the duplicate list
			//else storing it in the initialValues set.
			if (initialValues.contains(numbers.get(i))) {
				duplicateList.add(numbers.get(i));
			} else {
				initialValues.add(numbers.get(i));
			}
		}
